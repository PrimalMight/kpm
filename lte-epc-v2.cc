...
main (int argc, char *argv[])
{
  ...
  AnimationInterface anim ("my_animation.xml");
  for (uint32_t u = 0; u < ueNodes.GetN(); ++u)
    {
      anim.UpdateNodeDescription(ueNodes.Get(u),"UE:" + std::to_string(u));
    }

  for (uint32_t u = 0; u < enbNodes.GetN(); ++u)
    {
      anim.UpdateNodeDescription(enbNodes.Get(u),"eNB:" + std::to_string(u));
    }
  anim.UpdateNodeDescription(2, "MME");
  anim.UpdateNodeDescription(0, "PGW");
  anim.UpdateNodeDescription(1, "SGW");
  anim.UpdateNodeDescription(3, "Remote Host");
